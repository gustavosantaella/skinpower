	...
	<tbody>
		@foreach ($list->all() as $element)
			<tr>
				...
				<td>{{$element->phoneuser}}</td>
				<td>{{date('d-m-Y H:i:s a',strtotime($element->creacion))}}</td>
				<td>{{$element->pay}}</td>
				<td>$./{{$element->total}}</td>
				<td>
					<a href="{{ route('ver venta',[Crypt::encryptString($element->id),Crypt::encryptString($element->nameuser)]) }}" class="btn btn-primary font-weight-bold">Consultar</a>
				</td>
			</tr>
		@endforeach
	</tbody>
